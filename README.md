...
- **Immutability is key.**
All `Element` objects are immutable, and any operation on them is *thread safe*.
This guarantee is not made for any other classes.
- **Parsing is not dynamic.**
An element stores what it represents. No parsing is done when obtaining values.
